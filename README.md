...
# 5
    # 面向对象编程
        ...
        -面向对象基本概念
            ...
            静态属性：
                在定义属性的时候，加上关键字： static     ，此时就是静态属性。
                静态属性又什么用？
                    一般属性： 他的值隶属于该类的具体某个对象 ，也叫作实例的属性，对象就叫做实例。
                    静态属性： 只隶属于 类 本身，所有实例的 共有数据，也叫公共属性
                静态属性的使用：
                    类:: $静态属性名；
            静态方法：
                如果爱一个方法的前面加上static关键字，则变成了静态方法 ，
                隶属于类，也是许多实例的 公共方法
                该类调用静态方法中，里面不能使用$this
            ...
            构造方法：
                ...
                是类中的一个 特殊的   方法，其作用是实例化一个对象，，给该对象的属性赋值
                 1 该方法 的名字：  __construct();
                 2 该方法的必须是 普通方法，应该是 public

                 3 使用 this 关键字 给属性赋值
            析构方法：
                构造方法是 创建 对象时，会自动调用     
                析构方法是 销毁 对象时，会自动调用 __destruct
            ...
            继承：
                ...
                父类、子类：
                    已有类为父类，新建类为子类，父类也叫基类，子类也叫派生类

                    php跟大多面向对象一样是单继承模式，C++是多继承模式
            访问（权限）修饰符：
                在类中的成员，通常都可以在前面加上三个修饰符：
                public  公共的 共有的  公开的  
                protected 受保护的 
                private   私有的 
            parent:
                通常子类中调用父类  就用parent
            构造方法析构方法在继承中的表现
                子类没有定义 构造方法或析构方法 时，会自动调用父类的构造方法，因此实例子类时需按照父类的构造方法进行

                子类定义了 构造方法或析构方法 时，则不会调用父类的构造方法，但可以手动调用：parent::__construct()

            重写override
                 重写又叫做覆盖，将父类继承的来的属性和方法重写’定义‘就是重写

            最终类 final class
                通常 一个类没有特别申明，则别人 可以随意拿过来使用并对之扩展-------继承
            ...
            最终方法 final method 
                ...
                通常 一个方法没有特别声明，下级类就可以对其进行“覆盖”-----重写

                如果方法不希望被下级类覆盖，则可以将其声明为 最终方法
                形式
                    final public funciton 方法名(){}

    设计模式
        工厂模式
            在实际应用中，我们总需要去实例化很多很多的类--以得到对象
            则:
                我们可以设计出一个“工厂”（其实就是类），该工厂的作用就是为人们生产各种对象，这种工厂通常只要制定类名，就可以获得该类的一个对象。
        单例模式
            在实际应用中，对于某些类，使用时，从头到尾，都只需要一个对象，就可以完成所有任务
            则：
                某个类，只允许 “创建” 一个对象，即使进行多次创建，也只能得到一个对象
    抽象类、抽象方法
        抽象类：
            在正常定义类的前面，加上关键字“abstract" 就构成了抽象类
            注意： 不能实例化抽象类对象      
            作用：
                抽象类可以用来规范一些类的共同特征，但又不需要去对其进行实例化
                也就是说抽象类的使命就是专门做“父类”，子类就继承了他的特性，这就是规范作用             
        抽象方法：
            是一个没有方法体（也不含大括号）的方法定义“头”而已，前面加上 abstract，就是抽象方法
            比如： abstract function f1($a,$b);
            作用：
                跟抽象类一样，配合抽象类，来实现对下级类的“行为规范”
                即相当于要求下级类去完成该功能，但自己是不做的 
    PHP中的重载技术

        通常面向对象语言的重载技术，其基本语法是：
            在一个类中，有多个同名的方法，每个方法的参数不同，这种现象就是重载
            参数不同： 数量个数不同，或类型不同，或顺序不同
        但在 PHP中，一个类中，根据就不可以定义多个同名函数-- 这是语法错误    
        实际上，PHP 中的重载是另一个概念：
        属性重载： 如果使用一个不存在的属性，就会去自动调用类中预先定义好的某个方法去处理数据
        方法重载： 如果使用一个不存在的方法，就会去自动调用类中预先定义好的某个方法去处理该行为
        ...
        属性重载：： 
            ...
            就是在面对上述4种情形的属性使用场景中，该对象如何“应对”的问题
            如果属性不存在，使用语法是以下的情形：
                 取值   $=  对象->属性      =》自动调用类中的 __get()方法
                赋值    对象->属性 =xx 值    =》自动调用类中的 __set()方法
                判断是否存在 isset(对象->属性)  =》自动调用类中的 __isset()方法
                销毁 unset（对象->属性）        =》自动调用类中的 __unset()方法 
                前提是：类中要预先定义好这些方法

         方法重载：
            __call()  / __staticCall ()调用静态方法

        上述的方法都成为魔术方法：_get()、__set()、__isset()、__unset()、__call()、__staticCall  

    接口 interface:

        类： 有属性、有方法、有类常量
        抽象类： 有属性、有方法、有类常量、还有抽象方法（只有方法头）
        “更” 抽象类就是接口：有接口常量、还有抽象方法（只有方法头）
        可见接口中 ，只有两类最简单特性信息
            接口常量： 就是常量
            抽象方法： 只有方法头 -- 下级自然就应该实现它；
        作用：
            接口可以看做是抽象类的更高层面的“抽象规范”，不考虑接口接口常量，那么就相当只规定了下级类 要做什么---至于怎么做，没规定。
        ...
        形式：
            ...
            一个类可以继承一个父类，但可以继承多个接口

            interface 接口1 extends 接口2 {} 
            一个接口也可以继承另一份接口 （这叫单继承）
    有关类与对象的相关知识

        类的自动加载
            是指，在外面的页面中，并不需要去引入类文件，但程序会在需要的时候就去自动去“动态加载”该类进来。

            什么叫“需要一个类”？
                1 创建一个对象的时候 new 
                2 直接使用一个类名（操作静态属性 或静态方法）

            做法非常简单：
            做法1 ： __autoload() 魔术函数
                即 当程序需要一个类的时候，就会去调用该函数：该函数我们需要自己去定义并在其中写好加载类文件的通用语句
            做法2 ： spl_autoload__register 函数
                该函数的作用是： 用它注册（声明）多个可以用来代替__autoload函数作用的函数
                语法：
                    spl_autoload_register('函数名1');
                    其后也得需要定义这些函数的作用，跟——autoload 一样，此时就可以应对更多的情形

        对象深复制 clone
            浅克隆： 只能克隆对象中的 “非对象非资源” 数据

            深克隆： php中默认克隆是浅克隆，要想实现深克隆（一个对象的所有属性都彻底实现"复制"），就需要对该对象类使用魔术方法： __clone()

        对象遍历
            对象也是可以使用forEach语句进行遍历，但有两点要求：
                1 只能遍历属性
                2 只能遍历 看的到 的属性
